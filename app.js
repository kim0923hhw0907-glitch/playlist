...
let currentUser = localStorage.getItem('pl_session') || null;
let sharedPlaylists = [];
...
function loadUserData() {
    ...
    // Fall back to old unprefixed keys if user-specific data is empty
    if (songs.length === 0 && currentUser) {
        const oldSongs = JSON.parse(localStorage.getItem('pl_songs2'));
        if (oldSongs && oldSongs.length > 0) {
            songs = oldSongs;
            playlists = JSON.parse(localStorage.getItem('pl_playlists2')) || [];
        }
        const oldShared = JSON.parse(localStorage.getItem('pl_shared'));
        if (oldShared && oldShared.length > 0) sharedPlaylists = oldShared;
    }

    // Migrate existing shared playlists (add like/comment fields)
    let migrated = false;
    sharedPlaylists.forEach(sp => {
        if (sp.likes === undefined) { sp.likes = 0; sp.dislikes = 0; sp.likedBy = []; sp.dislikedBy = []; migrated = true; }
        if (!sp.comments) { sp.comments = []; migrated = true; }
    });
    if (migrated) localStorage.setItem(userKey('pl_shared'), JSON.stringify(sharedPlaylists));

    const savedUI = JSON.parse(localStorage.getItem(userKey('pl_ui')));
    if (savedUI) {
        uiSettings = savedUI;
    } else if (currentUser) {
        const oldUI = JSON.parse(localStorage.getItem('pl_ui'));
        if (oldUI) uiSettings = oldUI;
        else uiSettings = { bgImages: [], bgMode: 'single', bgInterval: 10, bgIndex: 0, accent: '#6c63ff', blur: 10, dim: 50, playBtnImage: '' };
    } else {
...
async function saveShared() {
    try {
        ...
        // localStorage 용량 초과 → 파일 데이터를 IndexedDB로 이동
        let moved = 0;
        for (const sp of sharedPlaylists) {
            for (const song of sp.songs) {
                if (song.fileData) {
                    const refId = (currentUser || 'anon') + '_shared_' + uid();
                    const buf = base64ToArrayBuffer(song.fileData.base64);
                    await dbPut(refId, buf, song.fileData.name, song.fileData.type);
                    song.fileRef = { id: refId, name: song.fileData.name, type: song.fileData.type };
...
async function confirmBatchAdd() {
    ...
    for (let i = 0; i < inputs.length; i++) {
        ...
        songs.push({
            id: uid(), title, artist,
            url: '', fileId: entry.fid, isLocal: true
        });
    }
    save();
    closeBatchModal();
...
document.getElementById('edit-form').addEventListener('submit', e => {
    ...
    if (pendingEditFile) {
        ...
        const fid = (currentUser || 'anon') + '_' + uid();
        dbPut(fid, pendingEditFile.data, pendingEditFile.name, pendingEditFile.type);
        song.fileId = fid;
        song.isLocal = true;
        song.url = '';
        pendingEditFile = null;
...
(function() {
    ...
    form.addEventListener('submit', async e => {
        ...
        for (const s of songsList) {
            ...
            if (s.isLocal && s.fileId) {
                try {
                    const stored = await dbGet(s.fileId);
...
function getDefaultProfile(username) {
    return { displayName: username, bio: '', banner: '', avatar: '', hearts: 0, heartedBy: [] };
}

function loadProfile(username) {
    try {
        const data = JSON.parse(localStorage.getItem('pl_profile_' + username));
        return data ? Object.assign(getDefaultProfile(username), data) : getDefaultProfile(username);
    } catch (_) {
...
function saveProfileData(username, data) {
    localStorage.setItem('pl_profile_' + username, JSON.stringify(data));
}
...
function toggleProfileEdit() {
    const profile = loadProfile(profileViewUser);
...
function saveProfile() {
    const profile = loadProfile(profileViewUser);
    profile.displayName = document.getElementById('profile-edit-name').value.trim() || profileViewUser;
    profile.bio = document.getElementById('profile-edit-bio').value.trim();
    saveProfileData(profileViewUser, profile);
    if (profileViewUser === currentUser) {
        document.getElementById('user-display').textContent = profile.displayName + '님';
    }
    closeProfileModal();
}
...
document.getElementById('profile-banner-file').addEventListener('change', function() {
    ...
    reader.onload = e => {
        ...
        saveProfileData(profileViewUser, profile);
        document.getElementById('profile-banner-img').style.backgroundImage = 'url(' + profile.banner + ')';
    };
    reader.readAsDataURL(file);
});
...
function heartProfile() {
    if (!currentUser) { alert('로그인 후 이용할 수 있습니다.'); return; }
    const profile = loadProfile(profileViewUser);
    if (!profile.heartedBy) profile.heartedBy = [];
    if (profile.heartedBy.includes(currentUser)) {
        profile.hearts = Math.max(0, (profile.hearts || 0) - 1);
        profile.heartedBy = profile.heartedBy.filter(u => u !== currentUser);
    } else {
        profile.hearts = (profile.hearts || 0) + 1;
        profile.heartedBy.push(currentUser);
    }
    saveProfileData(profileViewUser, profile);
    document.getElementById('profile-heart-count').textContent = profile.hearts;
...
async function applySharedPlaylist(id) {
    ...
    for (const ss of sp.songs) {
        ...
        if (!existing) {
            const newSong = { id: uid(), title: ss.title, artist: ss.artist, url: ss.url, fileId: null, isLocal: false };
...
function registerUser(username, password) {
    ...
    const users = JSON.parse(localStorage.getItem('pl_users_db')) || [];
    if (users.find(u => u.username === username)) {
        document.getElementById('login-error').textContent = '이미 존재하는 사용자 이름입니다.';
        return false;
    }
    const hash = btoa(password);
    users.push({ username, password: hash });
    localStorage.setItem('pl_users_db', JSON.stringify(users));
...
function loginUser(username, password) {
    ...
    if (!user) { document.getElementById('login-error').textContent = '존재하지 않는 사용자입니다.'; return false; }
    if (user.password !== btoa(password)) { document.getElementById('login-error').textContent = '비밀번호가 일치하지 않습니다.'; return false; }
    currentUser = username;
    localStorage.setItem('pl_session', username);
    loadUserData();
    document.getElementById('login-error').textContent = '';
    return true;
}
