...
const GENRE_LABELS = {
    pop:'??, rnb:'R&B', rock:'??, ballad:'Î∞úÎùº??, jpop:'JPop', kpop:'KPop',
    hiphop:'?ôÌï©', edm:'EDM', indie:'?∏Îîî', classic:'?¥Îûò??, jazz:'?¨Ï¶à',
    country:'Ïª®Ìä∏Î¶?, folk:'?¨ÌÅ¨', metal:'Î©îÌÉà', reggae:'?àÍ≤å', soul:'?åÏö∏',
    funk:'?ëÌÅ¨', blues:'Î∏îÎ£®??, other:'Í∏∞Ì?'
};

const DURATION_LABELS = {
    '30min':'30Î∂?, '1h':'1?úÍ∞Ñ', '2h':'2?úÍ∞Ñ', '3h':'3?úÍ∞Ñ', other:'Í∏∞Ì?'
};
...
function applyUI() {
    ...
    if (preview) {
        preview.innerHTML = uiSettings.playBtnImage
            ? '<div class="preview-circle" style="background-image:url(' + uiSettings.playBtnImage + ')"></div><span style="color:var(--text-secondary);font-size:0.82rem">Ïª§Ïä§?Ä ?¥Î?ÏßÄ ?ÅÏö©??/span>'
            : '<span style="color:var(--text-secondary);font-size:0.82rem">Í∏∞Î≥∏ Î≤ÑÌäº (Í∑∏Îùº?∞Ïù¥??</span>';
    }
...
function renderBgGallery() {
    ...
    const images = uiSettings.bgImages;
    if (images.length === 0) {
        gallery.innerHTML = '<p style="color:var(--text-secondary);font-size:0.82rem">?±Î°ù??Î∞∞Í≤Ω ?¥Î?ÏßÄÍ∞Ä ?ÜÏäµ?àÎã§</p>';
        return;
    }
...
function addBgImage() {
    ...
    if (!url) { alert('?¥Î?ÏßÄ URL???ÖÎ†•?òÍ±∞???åÏùº ?†ÌÉù/?úÎûòÍ∑∏Ìïò?∏Ïöî.'); return; }
...
function setPlayBtnImage() {
    ...
    if (!url) { alert('?¥Î?ÏßÄ URL???ÖÎ†•?òÍ±∞???åÏùº???úÎûòÍ∑∏Ìïò?∏Ïöî.'); return; }
...
function readImageFile(file, callback) {
    if (!file || !file.type.startsWith('image/')) { alert('?¥Î?ÏßÄ ?åÏùºÎß?ÏßÄ?êÌï©?àÎã§.'); return; }
...
libSection.addEventListener('drop', async e => {
    ...
    if (files.length === 0) {
        alert('?§Îîî???êÎäî ÎπÑÎîî???åÏùºÎß?Ï∂îÍ??????àÏäµ?àÎã§.');
        return;
    }
    if (files.length === 1) {
...
async function showBatchModal(files) {
    ...
    count.textContent = '(' + files.length + 'Í∞?';
    pendingBatch = [];
    list.innerHTML = '';
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        item.innerHTML =
            ...
            '<input class="batch-title" value="' + esc(title).replace(/"/g, '&quot;') + '" placeholder="?úÎ™©">' +
            '<input class="batch-artist" value="' + esc(artist).replace(/"/g, '&quot;') + '" placeholder="?ÑÌã∞?§Ìä∏">' +
            '</div>';
...
function renderLibrary() {
    ...
    filter.innerHTML = '<button class="chip' + (selectedArtist === 'all' ? ' active' : '') + '" data-filter="all">?ÑÏ≤¥</button>' +
    ...
    if (filtered.length === 0) {
        list.innerHTML = '<div class="empty-state">' + (songs.length === 0 ? '?Ä?•Îêú ?∏ÎûòÍ∞Ä ?ÜÏäµ?àÎã§. ?ÑÏóê???∏ÎûòÎ•?Ï∂îÍ??òÍ±∞???åÏùº???úÎûòÍ∑∏Ìïò?∏Ïöî.' : '???ÑÌã∞?§Ìä∏???∏ÎûòÍ∞Ä ?ÜÏäµ?àÎã§.') + '</div>';
        return;
    }
    list.innerHTML = filtered.map(s =>
        '<div class="card">' +
            ...
            '</div>' +
            '<div class="actions">' +
                '<button class="btn-small play-btn" onclick="playLibrarySong(\'' + s.id + '\')">?¨ÏÉù</button>' +
                '<button class="btn-small" onclick="editSong(\'' + s.id + '\')">?òÏ†ï</button>' +
                '<button class="btn-danger" onclick="deleteSong(\'' + s.id + '\')">??†ú</button>' +
            '</div>' +
        '</div>'
    ).join('');
}
...
function editSong(id) {
    ...
    document.getElementById('edit-drop-zone').querySelector('p').textContent = '???§Îîî???åÏùº???åÏñ¥???ìÏúº?∏Ïöî (?†ÌÉù)';
...
function renderEditLogoPreview(url) {
    ...
    if (url) {
        el.innerHTML = '<img src="' + esc(url) + '"><span style="color:var(--text-secondary);font-size:0.82rem">Î°úÍ≥† ?ÅÏö©??/span>';
    } else {
        el.innerHTML = '<span style="color:var(--text-secondary);font-size:0.82rem">Î°úÍ≥† ?ÜÏùå</span>';
    }
}
...
function renderPlaylists() {
    ...
    if (playlists.length === 0) {
        list.innerHTML = '<div class="empty-state">?ùÏÑ±???åÎ†à?¥Î¶¨?§Ìä∏Í∞Ä ?ÜÏäµ?àÎã§.</div>';
        return;
    }
        ...
        return '<div class="card playlist-card"><div style="width:100%">' +
            '<div class="playlist-header">' +
                '<div class="info" onclick="toggleExpandPlaylist(\'' + pl.id + '\')">' +
                    ...
                    '<div class="text-group">' +
                        '<h4>' + esc(pl.name) + '</h4>' +
                        '<p>' + pl.songs.length + 'Í≥?/p>' +
                    '</div>' +
                '</div>' +
                '<div class="actions">' +
                    (plSongs.length > 0 ? '<button class="btn-small play-btn" onclick="playPlaylist(\'' + pl.id + '\', 0)">?ÑÏ≤¥ ?¨ÏÉù</button>' : '') +
                    (availSongs.length > 0 ? '<button class="btn-small" onclick="event.stopPropagation();toggleQuickAdd(\'' + pl.id + '\')">+</button>' : '') +
                    '<button class="btn-small" onclick="event.stopPropagation();openEditPlaylistModal(\'' + pl.id + '\')">?òÏ†ï</button>' +
                    '<button class="btn-danger" onclick="deletePlaylist(\'' + pl.id + '\')">??†ú</button>' +
                '</div>' +
            '</div>' +
            '<div id="' + quickAddId + '" class="quick-add" style="display:none">' +
                '<select onchange="quickAddSong(\'' + pl.id + '\', this.value, this)">' +
                    '<option value="">-- Í≥??†ÌÉù --</option>' +
                    availSongs.map(s => '<option value="' + s.id + '">' + esc(s.title) + ' - ' + esc(s.artist) + '</option>').join('') +
                '</select>' +
            '</div>' +
            (expanded ? '<div class="playlist-body">' +
                '<ul class="playlist-songs">' +
                    (plSongs.length === 0 ? '<li style="color:var(--text-secondary);justify-content:center;border:none;padding:12px 0">Í≥°Ïù¥ ?ÜÏäµ?àÎã§</li>' :
                    plSongs.map((s, i) =>
                        '<li>' +
                            ...
                            '<div class="actions">' +
                                ...
                                '<button class="btn-small" onclick="playPlaylist(\'' + pl.id + '\', ' + i + ')">?¨ÏÉù</button>' +
                                '<button class="btn-small btn-danger" onclick="removeSongFromPlaylist(\'' + pl.id + '\', ' + i + ')" title="?úÍ±∞">??/button>' +
                            '</div>' +
                        '</li>'
                    ).join('')) +
                '</ul>' +
                (availSongs.length > 0 ?
                    '<div class="playlist-add">' +
                        '<select id="' + addId + '">' +
                            ...
                            availSongs.map(s => '<option value="' + s.id + '">' + esc(s.title) + ' - ' + esc(s.artist) + '</option>').join('') +
                        '</select>' +
                        '<button class="btn-primary" onclick="addSongFromSelect(\'' + pl.id + '\')">Ï∂îÍ?</button>' +
                    '</div>' :
                    '<p style="color:var(--text-secondary);margin-top:10px;font-size:0.85rem">Ï∂îÍ???Í≥°Ïù¥ ?ÜÏäµ?àÎã§</p>') +
            '</div>' : '') +
        '</div></div>';
    }).join('');
}
...
async function renderCommunity() {
    ...
    if (sel) {
        sel.innerHTML = '<option value="">-- ?åÎ†à?¥Î¶¨?§Ìä∏ ?†ÌÉù --</option>' +
            playlists.map(p => '<option value="' + p.id + '">' + esc(p.name) + ' (' + p.songs.length + 'Í≥?</option>').join('');
    }
    try {
        const serverData = await sbLoadShared();
...
function renderSharedPlaylists() {
    ...
    if (filterBar) {
        ...
        filterBar.innerHTML =
            '<div class="filter-section"><span class="filter-label">?•Î•¥</span>' +
            '<button class="chip' + (sharedGenreFilter === 'all' ? ' active' : '') + '" onclick="setSharedGenreFilter(\'all\')">?ÑÏ≤¥</button>' +
            genres.map(g => '<button class="chip' + (sharedGenreFilter === g ? ' active' : '') + '" onclick="setSharedGenreFilter(\'' + g + '\')">' + esc(getGenreLabel(g)) + '</button>').join('') +
            '</div>' +
            '<div class="filter-section"><span class="filter-label">Í∏∏Ïù¥</span>' +
            '<button class="chip' + (sharedDurationFilter === 'all' ? ' active' : '') + '" onclick="setSharedDurationFilter(\'all\')">?ÑÏ≤¥</button>' +
    ...
    if (filtered.length === 0) {
        list.innerHTML = '<div class="empty-state">' + (sharedPlaylists.length === 0 ? '?ÑÏßÅ Í≥µÏú†???åÎ†à?¥Î¶¨?§Ìä∏Í∞Ä ?ÜÏäµ?àÎã§.<br>?ÑÏóê??ÏßÅÏ†ë Í≥µÏú†?òÍ±∞??JSON ?åÏùº??Í∞Ä?∏Ïò§?∏Ïöî.' : 'Ï°∞Í±¥??ÎßûÎäî ?åÎ†à?¥Î¶¨?§Ìä∏Í∞Ä ?ÜÏäµ?àÎã§.') + '</div>';
        return;
    }

    list.innerHTML = filtered.map(sp => {
        const genreLabel = sp.genre === 'other' ? (sp.genreCustom || 'Í∏∞Ì?') : getGenreLabel(sp.genre);
        const durLabel = sp.duration === 'other' ? (sp.durationCustom || 'Í∏∞Ì?') : getDurationLabel(sp.duration);
        ...
        return '<div class="shared-card">' +
            ...
            '<div class="shared-card-meta">' +
                '<span class="song-count">' + sp.songs.length + 'Í≥?/span>' +
                '<div class="shared-card-rating">' +
                    '<button class="rate-btn' + (liked ? ' active' : '') + '" onclick="likeSharedPlaylist(\'' + sp.id + '\')" title="Ï¢ãÏïÑ??>?ëç <span id="like-cnt-' + sp.id + '">' + (sp.likes || 0) + '</span></button>' +
                    '<button class="rate-btn' + (disliked ? ' active' : '') + '" onclick="dislikeSharedPlaylist(\'' + sp.id + '\')" title="?´Ïñ¥??>?ëé <span id="dislike-cnt-' + sp.id + '">' + (sp.dislikes || 0) + '</span></button>' +
                '</div>' +
                '<div class="shared-card-actions">' +
                    '<button class="btn-small" onclick="applySharedPlaylist(\'' + sp.id + '\')">???ºÏù¥Î∏åÎü¨Î¶¨Ïóê Ï∂îÍ?</button>' +
                    '<button class="btn-small" onclick="document.getElementById(\'' + commId + '\').classList.toggle(\'visible\')">?ìÍ? ' + ((sp.comments && sp.comments.length) || 0) + '</button>' +
                    '<button class="btn-small" onclick="downloadSharedPlaylist(\'' + sp.id + '\')">?§Ïö¥Î°úÎìú</button>' +
                    '<button class="btn-small btn-danger" onclick="deleteSharedPlaylist(\'' + sp.id + '\')">??†ú</button>' +
                '</div>' +
            '</div>' +
            '<div id="' + commId + '" class="shared-comments">' +
                '<div class="comments-list">' +
                    (sp.comments && sp.comments.length
                        ...
                        : '<p class="no-comments">?ÑÏßÅ ?ìÍ????ÜÏäµ?àÎã§.</p>') +
                '</div>' +
                '<div class="comment-input-row">' +
                    '<input type="text" class="comment-input" id="' + commId + '-input" placeholder="?ìÍ? ?ëÏÑ±...">' +
                    '<button class="btn-small" onclick="addComment(\'' + sp.id + '\')">?ëÏÑ±</button>' +
                '</div>' +
            '</div>' +
        '</div>';
    }).join('');
...
(function() {
    ...
    form.addEventListener('submit', async e => {
        ...
        if (!pl) { alert('?åÎ†à?¥Î¶¨?§Ìä∏Î•??†ÌÉù?òÏÑ∏??'); return; }
        const songsList = getPlaylistSongs(plId);
        if (songsList.length === 0) { alert('?åÎ†à?¥Î¶¨?§Ìä∏??Í≥°Ïù¥ ?ÜÏäµ?àÎã§.'); return; }
        ...
        if (genre === 'other' && !genreCustom) { alert('?•Î•¥Î•?ÏßÅÏ†ë ?ÖÎ†•?òÏÑ∏??'); return; }
        ...
        if (duration === 'other' && !durationCustom) { alert('Í∏∏Ïù¥Î•?ÏßÅÏ†ë ?ÖÎ†•?òÏÑ∏??'); return; }
        ...
        alert('?åÎ†à?¥Î¶¨?§Ìä∏Í∞Ä Í≥µÏú†?òÏóà?µÎãà?? (' + fileCount + 'Í≥??åÏùº ?¨Ìï®)');
    });
...
async function downloadSharedPlaylist(id) {
    ...
    a.href = url;
    a.download = sp.title.replace(/[^a-zA-Z0-9Í∞Ä-??/g, '_') + '.json';
    a.click();
...
async function importSharedPlaylist() {
    ...
    if (!file) { alert('JSON ?åÏùº???†ÌÉù?òÏÑ∏??'); return; }
    try {
        ...
        if (!data.title || !data.author || !Array.isArray(data.songs)) {
            throw new Error('?¨Î∞îÎ•??åÎ†à?¥Î¶¨?§Ìä∏ ?ïÏãù???ÑÎãô?àÎã§.');
        }
        data.id = uid();
        ...
        alert('?åÎ†à?¥Î¶¨?§Ìä∏Î•?Í∞Ä?∏Ïôî?µÎãà??' + (fileCount ? ' (' + fileCount + 'Í≥??åÏùº ?¨Ìï®)' : ''));
    } catch (err) {
        alert('?åÏùº???ΩÏùÑ ???ÜÏäµ?àÎã§: ' + err.message);
    }
}
...
async function deleteSharedPlaylist(id) {
    ...
    if (sp.sharedBy && sp.sharedBy !== currentUser) {
        alert('?êÏã†??Í≥µÏú†???åÎ†à?¥Î¶¨?§Ìä∏Îß???†ú?????àÏäµ?àÎã§.');
        return;
    }
    if (!confirm('??Í≥µÏú† ?åÎ†à?¥Î¶¨?§Ìä∏Î•???†ú?òÏãúÍ≤†Ïäµ?àÍπå?')) return;
...
function likeSharedPlaylist(id) {
    const sp = sharedPlaylists.find(p => p.id === id);
    if (!sp) return;
    if (!currentUser) { alert('Î°úÍ∑∏?????¥Ïö©?????àÏäµ?àÎã§.'); return; }
...
async function applySharedPlaylist(id) {
    ...
    if (added.length === 0) { alert('Î™®Îì† Í≥°Ïù¥ ?¥Î? ?ºÏù¥Î∏åÎü¨Î¶¨Ïóê ?àÏäµ?àÎã§.'); return; }

    const pl = { id: uid(), name: sp.title + ' (Í≥µÏú†)', songs: added.map(s => s.id) };
    playlists.push(pl);
    save();
    renderLibrary();
    renderPlaylists();
    alert('?åÎ†à?¥Î¶¨?§Ìä∏ "' + pl.name + '"Í∞Ä ?ºÏù¥Î∏åÎü¨Î¶¨Ïóê Ï∂îÍ??òÏóà?µÎãà?? (' + added.length + 'Í≥?');
}
        ...
        if (song.isLocal && song.filePath) {
            if (song.filePath.startsWith('idxdb:')) {
                ...
                if (fileData) {
                    ...
                    throw new Error('?åÏùº??Ï∞æÏùÑ ???ÜÏäµ?àÎã§');
                }
            } else {
                try {
                    ...
                    throw new Error('?åÏùº??Î∂àÎü¨?????ÜÏäµ?àÎã§');
                }
            }
        } else if (song.fileId) {
            // Legacy: fileId-based
...
function updatePlayerUI() {
    ...
    document.getElementById('current-title').textContent = song ? song.title : '?†ÌÉù???∏Îûò ?ÜÏùå';
    ...
    if (queue.length > 1 && queueIndex < queue.length - 1) {
        ...
        nextEl.innerHTML = '?§Ïùå Í≥? <b>' + esc(next.title) + '</b> ??' + esc(next.artist);
    } else {
        nextEl.textContent = '';
    }
...
function renderPlayer() {
    ...
    document.getElementById('queue-count').textContent = '(' + queue.length + 'Í≥?';
    ...
    if (sel) {
        ...
        sel.innerHTML = '<option value="">-- Í≥??†ÌÉù --</option>' +
...
async function registerUser(username, password) {
    ...
    if (username.length < 2) { document.getElementById('login-error').textContent = '?¨Ïö©???¥Î¶Ñ?Ä 2???¥ÏÉÅ?¥Ïñ¥???©Îãà??'; return false; }
    if (password.length < 4) { document.getElementById('login-error').textContent = 'ÎπÑÎ?Î≤àÌò∏??4???¥ÏÉÅ?¥Ïñ¥???©Îãà??'; return false; }

    // Save locally first (instant)
    const users = JSON.parse(localStorage.getItem('pl_users')) || [];
    if (users.some(u => u.username === username)) {
        document.getElementById('login-error').textContent = '?¥Î? Ï°¥Ïû¨?òÎäî ?¨Ïö©?êÏûÖ?àÎã§.';
        return false;
    }
    users.push({ username, password });
    localStorage.setItem('pl_users', JSON.stringify(users));
    document.getElementById('login-error').textContent = '?åÏõêÍ∞Ä???±Í≥µ! Î°úÍ∑∏?∏Ìï¥ Ï£ºÏÑ∏??';
...
async function loginUser(username, password) {
    ...
    if (match && match.password !== password) {
        document.getElementById('login-error').textContent = 'ÎπÑÎ?Î≤àÌò∏Í∞Ä ?Ä?∏Ïäµ?àÎã§.';
        return false;
    }
    ...
    if (!match && !hasUserSpecificData) {
        document.getElementById('login-error').textContent = 'Ï°¥Ïû¨?òÏ? ?äÎäî ?¨Ïö©?êÏûÖ?àÎã§. ?åÏõêÍ∞Ä?ÖÌï¥ Ï£ºÏÑ∏??';
        return false;
    }
...
async function handleLoginClick() {
    ...
    try {
        ...
        if (!username) { document.getElementById('login-error').textContent = '?¨Ïö©???¥Î¶Ñ???ÖÎ†•?òÏÑ∏??'; loginInProgress = false; return; }
        if (!password) { document.getElementById('login-error').textContent = 'ÎπÑÎ?Î≤àÌò∏Î•??ÖÎ†•?òÏÑ∏??'; loginInProgress = false; return; }
        const ok = await loginUser(username, password);
        if (ok) showApp();
    } catch (e) {
        console.error('handleLoginClick error:', e);
        document.getElementById('login-error').textContent = '?§Î•ò: ' + e.message;
    }
